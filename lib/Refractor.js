...
function Refractor(props) {
  if (process.env.NODE_ENV !== 'production') {
    if (!fract.registered(props.language)) {
      ...
      console.warn('No language definitions for "' + props.language + '" seems to be registered, did you forget to call `Refractor.registerLanguage()`?');
    }
  }

  var langClassName = 'language-' + props.language;
